...
// 배포/로컬 불일치 방지용
const getAuthOrigin = (c) => c.env.AUTH_ORIGIN || 'http://localhost:8787'
...
app.get('/', async (c) => {
  ...
  url.searchParams.set('redirect_uri', callbackUrl) // authorize와 token 교환 모두 '완전 동일'
...
app.get('/callback', async (c) => {
  ...
  let tokenJson
  try { tokenJson = JSON.parse(tText) } catch {

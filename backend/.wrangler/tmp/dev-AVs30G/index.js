...
// .wrangler/tmp/bundle-152zTe/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-152zTe/middleware-insertion-facade.js
init_modules_watch_stub();
...
app.get("/callback", async (c) => {
  ...
  const { error } = await supa.from("users").upsert(row, { onConflict: "provider_sub" });
  if (error) {
...
app2.get("/", async (c) => {
  ...
  const { data, error } = await supa.from("users").select("id, provider, provider_sub, nickname, email, photo_url").eq("provider", provider).eq("provider_sub", providerSub).single();

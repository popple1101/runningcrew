...
// .wrangler/tmp/bundle-icLN5C/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-icLN5C/middleware-insertion-facade.js
init_modules_watch_stub();
...
var app = new Hono2();
var makeCallback = /* @__PURE__ */ __name((c) => {
  const origin = new URL(c.req.url).origin;
  return `${origin}/auth/kakao/callback`;
}, "makeCallback");
var pickKakaoCallback = /* @__PURE__ */ __name((c) => {
...
app.get("/", async (c) => {
  ...
  authUrl.searchParams.set("redirect_uri", pickKakaoCallback(c));
...
app.get("/callback", async (c) => {
  ...
  form.set("redirect_uri", pickKakaoCallback(c));
  form.set("code", code);
  ...
  const row = { provider: "kakao", provider_sub: kakaoId, nickname, email, photo_url, last_login_at: (/* @__PURE__ */ new Date()).toISOString() };
...
app5.put("/", async (c) => {
  ...
  }).eq("id", userId).select("id, nickname").single();
  if (error) {

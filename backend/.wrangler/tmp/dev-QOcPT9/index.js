...
// .wrangler/tmp/bundle-KUOCof/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-KUOCof/middleware-insertion-facade.js
init_modules_watch_stub();
...
app.get("/callback", async (c) => {
  ...
  const photo_url = me.kakao_account?.profile?.thumbnail_image_url || me.kakao_account?.profile?.profile_image_url || null;
  const supa = getSupabase(c);
  const { data: user, error } = await supa.from("users").upsert({ provider, provider_sub, nickname, email, photo_url }, { onConflict: "provider_sub" }).select("*").eq("provider_sub", provider_sub).single();
...
app2.get("/", async (c) => {
  ...
  try {
    ...
    const supa = getSupabase(c);
    const { data } = await supa.from("users").select("id,nickname,photo_url").eq("id", payload.sub).single();
...
// src/index.js
var app3 = new Hono2();
app3.get("/", (c) => c.json({ ok: true, name: "RunCrew API" }));
app3.route("/auth/kakao", kakao_default);
app3.route("/me", session_default);

...
// .wrangler/tmp/bundle-GbWzLt/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-GbWzLt/middleware-insertion-facade.js
init_modules_watch_stub();
...
app.get("/callback", async (c) => {
  ...
  try {
    ...
    const { data: user, error } = await supa.from("users").upsert(
      { provider, provider_sub, nickname, email, photo_url },
      { onConflict: "provider_sub" }
    ).select("*").eq("provider_sub", provider_sub).single();
...
app2.get("/", async (c) => {
  ...
  const supa = getSupabase(c);
  const { data, error } = await supa.from("users").select("id,nickname,photo_url").eq("id", payload.sub).single();
  if (error) return c.text("DB Error: " + error.message, 500);
